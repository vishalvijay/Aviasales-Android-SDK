...
public class PayTypeAdapter extends BaseExpandedListViewAdapter {
	...
	public PayTypeAdapter(Context context, List<BaseCheckedText> items, boolean hideTitle) {
	...
	public View getItemView(View view, ViewGroup parent, int position) {
		...
		BaseCheckedText payType = (BaseCheckedText) getItem(position);

		((BaseFiltersListViewItem) view).setCheckedText(payType);
		return view;
	}
	...
	public ViewGroup getTitleView(View view, ViewGroup parent) {
		...
		((TextView) view.findViewById(R.id.txtv_airports_view_list_title)).setText(R.string.pay_type);
